...
def save_file(request, session, last_upload, app):
    user = ''
    ...
    if file and allowed_file(file.filename):  # and min_last_upload > UPLOAD_RATE_LIMIT_MINUTES:
        valid_upload(request, session, user, file, last_upload, app)
...
def valid_upload(request, session, user, file, last_upload, app):
    u = uuid.uuid4()
    filename = str(u) + '.gz'
    if user == '':
        user_id = -1
